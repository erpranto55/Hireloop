...
export default async function CompaniesPage() {
  let dbCompanies: any[] = [];
  try {
    const res = await fetch("http://localhost:5001/api/companies?status=approved", { cache: "no-store" });
    if (res.ok) {
      ...
      <section className="mx-auto max-w-7xl pb-20">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {dbCompanies.map((company) => {
            ...
            return (
              <article id={compId} key={compId} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 transition hover:border-indigo-500/40 hover:bg-white/[0.06]">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-white p-2">
                    <Image src={company.logo || "/logos/nvidia.png"} alt={`${company.name} logo`} width={42} height={42} className="h-auto w-auto object-contain" />
                  </div>
                ...
                <div className="mt-6 space-y-3 text-sm text-zinc-400">
                  ...
                  <p className="flex items-center gap-2"><Building2 size={16} /> {company.openJobs || 0} open jobs</p>
                </div>

                <Link href={`/jobs?company=${compId}`} className="mt-6 inline-flex text-sm font-medium text-indigo-300 transition hover:text-indigo-200">
                  View open roles
                </Link>
              </article>
            );
