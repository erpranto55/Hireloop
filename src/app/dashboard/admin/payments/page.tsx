...
const payments = [
  ["OrbitAI", "Growth", "$49.00", "Paid"],
  ["Northstar Fintech", "Enterprise", "$149.00", "Paid"],
  ["PixelCart", "Pro", "$19.00", "Pending"],
  ["DevForge", "Premium", "$39.00", "Paid"],
];
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        />
        <button className="inline-flex w-fit items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-sm font-medium text-zinc-200 hover:bg-white/10">
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-white/3">
        <table className="w-full min-w-175 text-left text-sm">
          ...
          <tbody className="divide-y divide-white/10">
            {payments.map(([customer, plan, amount, status]) => (
              <tr key={customer}>
                <td className="font-medium text-white">{customer}</td>
                <td className="text-zinc-400">{plan}</td>
                <td className="text-zinc-200">{amount}</td>
                <td>
                  <span
                    className={`rounded-full border px-3 py-1 text-xs ${status === "Paid" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-amber-500/30 bg-amber-500/10 text-amber-300"}`}
                  >
                    {status}
                  </span>
                </td>
